...
app.post("/login/", async (request, response) => {
  ...
  const dbUserExit = await database.get(checkUser);
  if (dbUserExit !== undefined) {
    const checkPassword = await bcrypt.compare(password, dbUserExit.password);
...
app.get(
  ...
  async (request, response) => {
    ...
    on user.user_id= tweet.user_id where user.user_id in (${getFollowerIdsSingle})
...
app.get("/tweets/:tweetId/", authenticationToken, async (request, response) => {
  ...
  const getTweetIdsArray = await database.all(getTweetIdsArray);
  ...
  if (followingTweetIds.includes(parseInt(tweetId))) {
    const likes_count_query = `select count(user_id) as likes from where tweet_id=${tweetId};`;
...
app.get(
  ...
  async (request, response) => {
    ...
    const getFollowingIdsQuery = `select following_user_id from follower where follower_user_id=${getUseerId.user_id};`;
    const getFollowingIdsArray = await database.all(getFollowingIdsQuery);
    //console.log(getFollowingIdsArray);
...
app.get(
  ...
  async (request, response) => {
    ...
    const getUserId = await database.get(getUserIdQuery);
    //console.log(getUserId);
    //get the ids of the whom the user is following
    const getFollowingIdsQuery = `select following_user_id from follower where follower_user_id=${getUserId.user_id};`;
    const getFollowingIdsArray = await database.all(getFollowingIdsQuery);
    //console.log(getFollowingIdsArray);
    const getFollowingIds = getFollowingIdsArray.map((eachFollower) => {
...
app.get("/user/tweets/", authenticationToken, async (request, response) => {
  ...
  const getTweetIdsQuery = `select user_id from tweet where user_id=${getUserId.user_id};`;
  const getTweetIdsArray = await database.all(getTweetIdsQuery);
...
app.post("/user/tweets/", authenticationToken, async (request, response) => {
  ...
  response.send("Creates a Tweet");
});

/* 
// to check if the tweet got updated
app.get("/tweet/", authenticationToken, async (request, response) => {
...
app.delete(
  ...
  async (request, response) => {
    ...
    const getUserTweetsListQuery = `select tweeet_id from tweet where user_id=${getUserId.user_id};`;
